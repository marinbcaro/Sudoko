...
def naked_twins(values):
    ...
    for unit in unitlist:
        ...
        for box in unit:
            if len(values[box]) == 2:
                if values[box] not in candidates:
                    candidates[values[box]] = [box]
                else:
                    candidates[values[box]].append(box)
        for key in candidates:
            if len(candidates[key]) == 2:
                if key not  in naked_twins:
                    naked_twins[key] = [unit]
                else:
                    naked_twins[key].append(unit)
    for k in naked_twins:
        for unit in naked_twins[k]:
            for box in unit:
                if values[box] != k:
